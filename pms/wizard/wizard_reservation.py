...
class FolioWizard(models.TransientModel):
    ...
    reservation_wizard_ids = fields.One2many('pms.reservation.wizard',
                                             'folio_wizard_id',
                                             string="Resevations")
    service_wizard_ids = fields.One2many('pms.service.wizard',
                                         'folio_wizard_id',
                                         string='Services')
    ...
    room_type_wizard_ids = fields.One2many('pms.room.type.wizard',
                                           'folio_wizard_id',
                                           string="Room Types")
    ...
    def create_reservations(self):
        ...
        for line in self.room_type_wizard_ids:
            ...
            if line.rooms_num > line.max_rooms:
                ...
                room_list = self.env['pms.room'].search([
                    ('id', 'in', room_ids)
                ])
                ...
                for room in room_list:
                    adults = self.env['pms.room'].search([
                        ('id', '=', room.id)
                    ]).capacity
                    cmds.append((0, False, {
    ...
    def domain_board_service(self):
        for line in self:
            ...
            if line.room_type_id:
                ...
                board_services_room_type = self.env['pms.board.service.room.type'].\
                    search([('pms_room_type_id', '=', self.room_type_id.id),
