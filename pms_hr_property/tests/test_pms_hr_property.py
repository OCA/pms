...
class TestPmsHrProperty(TransactionCase):
    def setUp(self):
        ...
        # Create users
        user_dict = {"name": "User 1", "login": "tua@example.com", "password": "base-test-passwd"}
        self.user_test = self.env["res.users"].create(user_dict)

        user_dict2 = {"name": "User 2", "login": "user2@example.com", "password": "base-test-passwd"}
        self.user_test2 = self.env["res.users"].create(user_dict2)

        # Create employees and link them to users
        employee_dict = {"name": "Employee 1", "user_id": self.user_test.id, "address_id": self.user_test.partner_id.id}
        self.employee = self.env["hr.employee"].create(employee_dict)

        employee_dict2 = {"name": "Employee 2", "user_id": self.user_test2.id, "address_id": self.user_test2.partner_id.id}
    ...
    def test_employee_assignment(self):
        ...
        # Verify the employee is correctly assigned
        assigned_employees = self.property1.employee_ids
        self.assertIn(self.employee, assigned_employees, "The employee is not correctly assigned to the property.")

    def test_employee_removal(self):
        """Check if employees can be unassigned from properties correctly"""
        # Assign employee to the property
        self.employee.write({"property_ids": [(4, self.property1.id)]})

        # Unassign employee from the property
        self.employee.write({"property_ids": [(3, self.property1.id)]})

        # Verify the employee is no longer assigned to the property
        assigned_employees = self.property1.employee_ids
        self.assertNotIn(self.employee, assigned_employees, "The employee is still assigned to the property after removal.")

    def test_multiple_employees_assignment(self):
        """Check if multiple employees can be assigned to a single property"""
        # Assign both employees to the property
        self.employee.write({"property_ids": [(4, self.property1.id)]})
        self.employee2.write({"property_ids": [(4, self.property1.id)]})

        # Verify both employees are assigned
        assigned_employees = self.property1.employee_ids
        self.assertIn(self.employee, assigned_employees, "Employee 1 is not correctly assigned.")
        self.assertIn(self.employee2, assigned_employees, "Employee 2 is not correctly assigned.")